 ...
 *  pruebaGSL
 *
 *  Created by Rafael Verduzco Vázquez on 2/1/11.
...
int disMap(const int i, const int * networkState) {
	if (i == 0) {
		return networkState[1];
	} else if (i == 1) {
		return networkState[0] * ( (networkState[2] + 1) %2);
	} else if (i == 2) {
		return networkState[1];
	} else {
		fprintf(stderr, "Wrong node index passed to discrete mapping\n");
		exit(5);
	}
}


int chucherias(const int i, const int * st)
{
	...
	switch (i) {
		...
		case 4: //pK
			...
			break;
		case 5: //v
			// Transform the key form ternary to decimal representation. Use that key to lookup in the hash table
...
/*
 int H(const int i, const double * contState, const double * thresholds) {
